...
# nmコマンドの情報をキャッシュする
my $path = [grep {-d $_} ("$sdk/lib", "$develop/lib/RHEL6.5")];
push @$path, "$workspace/*/Debug" unless @$path;

my $nmbuf = [];
my $nm = "nm -o " . join(' ', map{"$_/*.so"} @$path);
debug("[sh] $nm");
foreach(`$nm`) {
    my($so, $type, $name) = /^(.+):[0-9a-f]{16} (\w) (\w+)/;
    next unless $name;
    push @$nmbuf, {so=>$so, type=>$type, name=>$name};
}
debug("line $#$nmbuf");
...
while(my($class, $s) = each %$stub) {
    ...
    foreach my $func(keys %{$s->{func}}) {
        ...
        my $reg = join('\w+', split('::', $class), $func);
        foreach my $n(@$nmbuf) {
            my $name = $n->{name};
            next if $f->{nm}->{$name} || $name !~ /$reg/;
            my($method) = `c++filt -i $name`;
            debug("[next] $class\::$func != $method >> $n->{so}"),next unless $method =~ /$class\::$func.*?\(.*\)$/;
            $f->{nm}->{$name} = $n;
            $f->{nm}->{$name}->{lex} = new LexicalAnalyzer({code=>$method});
            $f->{nm}->{$name}->{method} = $method;
        }
...
sub find_define {
    ...
    foreach(@$find) {
        ...
        for(my $t = $_->{this}->next(); !$t->eof($end); $t = $t->next()) {
            ...
            next if /^op/ && $t->text() =~ /[,:]/;
            last if /^op/;
            next if /^ident/ && $t->text() =~ /^(public|protected|private)$/;
